...
def test_ping_occurs(pingboy, mock_ping_succeed, mock_restart_thread):
    """Confirms that a ping occurs."""
    pingboy.ping()

    mock_ping_succeed.assert_called_once_with('ping -c 1 192.168.1.39')

def test_ping_restart_successful(pingboy, mock_ping_succeed, mock_restart_thread, mock_my_logger):
...
def test_ping_restart_in_progress(pingboy, mock_ping_fail, mock_restart_thread, mock_my_logger):
...
def test_ping_failure_under_threshold(pingboy, mock_ping_fail, mock_restart_thread, mock_my_logger):
...
def test_ping_failure_over_threshold(pingboy, mock_ping_fail, mock_restart_thread, mock_my_logger):
    ...
    mock_my_logger.logger.error.assert_called_once_with("192.168.1.39 is down, requesting auto restart! Response = 1")
